...
// This ensures that when a due is toggled, ONLY this row instantly re-renders.
// ============================================================================
const DueRowUI = ({ due, isLast, onToggle }: { due: Due, isLast: boolean, onToggle: (due: Due) => void }) => (
  <TouchableOpacity
    style={[styles.dueRow, !isLast && styles.borderBottom]}
    activeOpacity={0.7}
  ...
  >
    ...
    <View style={[styles.checkbox, due.isPaid && styles.checkboxActive]}>
...
const PaymentsScreenUI = ({ navigation, expenses, dues }: PaymentsScreenProps) => {
  ...
  // Load just the budget setting from AsyncStorage
  useEffect(() => {
  ...
  const daysLeft = 5;
  const safeToSpend = remainingBudget / daysLeft;
  ...
  const saveExpense = async () => {
    ...
    await database.write(async () => {
      await database.get<Expense>('expenses').create(expense => {
        expense.title = expenseTitle || selectedCategory.name;
  ...
  const saveDue = async () => {
    ...
    await database.write(async () => {
      await database.get<Due>('dues').create(due => {
        due.title = dueTitle;
        due.amount = parseFloat(dueAmount);
        due.isPaid = false;
      });
    });
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.cardContainer}>
          {expenses.length === 0 ? (
            <Text style={styles.emptyText}>No expenses logged yet. Great job.</Text>
          ) : (
        ...
        <View style={styles.cardContainer}>
          {dues.length === 0 ? (
            <Text style={styles.emptyText}>No pending school fees.</Text>
          ) : (
            dues.map((due, index) => (
              // Using our new reactive component here
              <EnhancedDueRow
                key={due.id}
                due={due}
...
// Wire it all up to the offline database
...
const styles = StyleSheet.create({
  ...
  // --- Hero Card ---
  heroCard: { backgroundColor: palette.primary, borderRadius: 32, padding: spacing.xl, paddingTop: spacing.xxl, marginBottom: spacing.xxl, shadowColor: palette.primary, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 20, elevation: 10 },
  ...
  sectionLabel: { fontSize: 13, fontWeight: '700', color: palette.ink, marginTop: spacing.xl, marginBottom: spacing.md, letterSpacing: 0.8, textTransform: 'uppercase', opacity: 0.6 },
